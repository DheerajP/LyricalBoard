...
const lyricPage = (props) => {
  ...
  return (
    <>
      <Head>
        <title>{props.lyrics.internalName}</title>
      </Head>

      <div className="p-20 flex flex-col space-y-4 h-screen items-center">

        <h1 className="text-3xl mb-4 font-bold">{props.lyrics.internalName}</h1>

        <h4>Lyric:</h4>
        <div style={{ whiteSpace: "pre-wrap" }}>
          <ReactMarkdown>{props.lyrics.lyric}</ReactMarkdown>
        </div>
        <hr />

        <h4>My perspective:</h4>
        <div style={{ whiteSpace: "pre-wrap" }}>
          <ReactMarkdown>{props.lyrics.yourPerspective}</ReactMarkdown>
        </div>
        <hr />

        <h4>Writer:</h4>
        <p><strong>Name : </strong> <a href={'/writer/' + props.lyrics.lyricWriter.slug} >  {props.lyrics.lyricWriter.name}</a></p>
        <div style={{ whiteSpace: "pre-wrap" }}>
          <ReactMarkdown>{props.lyrics.lyricWriter.about}</ReactMarkdown>
        </div>
        {/* <div className="w-full rounded shadow-xl">
          <ImageComponent image={props.lyrics.lyricWriter.profilePic} />
        </div> */}


        <hr />
        <h4>Song:</h4>
        <p><strong>Name : </strong> <a href={'/song/' + props.lyrics.song.slug} >  {props.lyrics.song.name}</a></p>
        <p><strong>Movie : </strong> {props.lyrics.song.movie}</p>
        <div style={{ whiteSpace: "pre-wrap" }}>
          <ReactMarkdown>{props.lyrics.song.fullLyrics}</ReactMarkdown>
        </div>
...
export async function getStaticProps(context) {
  ...
  var data = JSON.stringify({
    query: `query GetLyricBySlug($slug: String!) {
      lyricsCollection(limit: 1, where: { slug: $slug }) {
        total
        items {
          internalName
          lyric
  ...
  return {
    props: {
      lyrics: responseData.data.lyricsCollection.items[0],
    },
  }
}
...
export async function getStaticPaths() {
  const lyricEntries = await getEntriesByContentType("lyrics");
