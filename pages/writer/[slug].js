...
const writerPage = (props) => {
  ...
  return (
    <>
      ...
      <Head>
        <title>{props.writer.name}</title>
      </Head>

      <div className="">

        <h1 className="text-3xl mb-4 font-bold">{props.writer.name}</h1>
        <div style={{ whiteSpace: "pre-wrap" }}>
          <ReactMarkdown>{props.writer.about}</ReactMarkdown>
        </div>
        <div className="">
          <ImageComponent image={props.writer.profilePic} />
        </div>
...
export async function getStaticProps(context) {
  ...
  var data = JSON.stringify({
    query: `query GetWriterBySlug($slug: String!) {
      writerCollection(limit: 1, where: { slug: $slug }) {
        total
        items {
            slug
            name
  ...
  return {
    props: {
      writer: responseData.data.writerCollection.items[0],
    },
  }
}
...
export async function getStaticPaths() {
  const lyricEntries = await getEntriesByContentType("writer");
